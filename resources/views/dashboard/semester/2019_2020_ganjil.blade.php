...
@section('content')
    ...
    <script>
        ...
        // function updateData() {
        // const matkul = [];
        // fetch('https://layar.yarsi.ac.id/webservice/rest/server.php?wstoken=463cfb78c5acc92fbed0656c2aec27b4&wsfunction=mod_resource_get_resources_by_courses&moodlewsrestformat=json')
        //     .then(response => response.json())
        //     .then(data => {
        //     for (let i = 0; i < data.resources.length; i++) {
        //         if (data.resources[i]['course'] === 7661) {
        //         const namaMatkul = data.resources[i]['name'];
        //         matkul.push(namaMatkul);
        //         }
        //     }
        //     console.log(matkul);
        ...
        function updateData() {
            ...
            $.ajax({
                type: 'GET',
                dataType: 'json',
                url: 'https://layar.yarsi.ac.id/webservice/rest/server.php?wstoken=463cfb78c5acc92fbed0656c2aec27b4&wsfunction=mod_resource_get_resources_by_courses&moodlewsrestformat=json',

                success: function (data, status, xhr) {
                    for (let i = 0; i < data.resources.length; i++) {
                        if (data.resources[i]['course'] === 7661 && data.resources[i]['contentfiles'] != null) {
                            const namaMatkul = data.resources[i]['name'];
                            const section = data.resources[i]['section'];
                            matkul.push([namaMatkul, section]);

                        }
                    }
                    console.log(matkul, matkul.length);
                },
                error: function (xhr, status, error) {
                    // penanganan kesalahan saat permintaan AJAX gagal
                    console.log('Error:', error);
